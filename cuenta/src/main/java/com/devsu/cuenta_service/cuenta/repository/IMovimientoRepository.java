...
public interface IMovimientoRepository extends IRepository<Movimiento, Integer> {
	...
	/*
	 * 
	 * @Query(value = """ SELECT c.*, m.*, CASE c.tipo_cuenta WHEN 'CORRIENTE' THEN
	 * 'CUENTA CORRIENTE' WHEN 'AHORRO' THEN 'CUENTA DE AHORRO' WHEN 'DEBITO' THEN
	 * 'CUENTA DE DEBITO' ELSE 'Desconocido' END AS tipo_cuenta_modificado FROM
	 * devsucuentadb.cuenta c JOIN devsucuentadb.movimientos m ON m.cuenta_id =
	 * c.id_cuenta WHERE m.fecha BETWEEN :fechaInicio AND :fechaFin AND c.cliente_id
	 * = :idCliente
	 * 
	 * 
	 * """, nativeQuery = true) List<Object[]>
	 * listarMovimientosPorClienteYFecha(@Param("idCliente") Integer idCliente,
	 * 
